...
Sparse-Period Kernel --- inside a single configuration that
generalizes across eight benchmarks without per-dataset tuning.
...
close; this is the deliberate trade-off behind the single-config
narrative.
...
First, lifting the single-configuration constraint --- letting
$P$ and the gate spectrum bin count adapt to a dataset's
dominant period --- without sacrificing the deployability
guarantee. Second, multimodal sensor fusion: combining the
