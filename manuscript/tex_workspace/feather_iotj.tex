...
% R2 #16: justify epoch count + no early stopping.
We keep the epoch budget fixed at $50$ rather than applying early
stopping; this trades a small amount of compute for the guarantee
that every (model, dataset, horizon) cell sees the same number of
gradient updates, which we view as more reproducible than per-cell
stopping decisions. All baselines retain their official validation
schedule for learning-rate decay and final-checkpoint selection on
the validation MSE.
...
budget) cell (Section~\ref{subsec:edge-estimator}), and an outline
of the \emph{QEMU validation path} that we use to calibrate the
estimator against cycle-accurate execution
...
\texttt{-nographic -semihosting}), which provides cycle-accurate
emulation of the Cortex-M3 core. The firmware is cross-compiled
with \texttt{arm-none-eabi-gcc}~12.2.1
(\texttt{-Os -mthumb -mcpu=cortex-m3}), links against CMSIS-NN, and
exposes the cycle counter through the DWT register so the cycle
count of each inference is read back over the semi-hosting channel.
...
\emph{Reporting policy.} The numbers in Table~\ref{tab:edge-etth1}
and Table~\ref{tab:edge-counts} are estimates, and we say so
explicitly. The companion QEMU experiment confirms that the
estimator's latency for FEATHer agrees with the cycle-accurate run
within the precision required for the deployability claim. Future
work includes a measurement on a physical LM3S6965EVB development
board with energy instrumentation; we expect the on-board energy
figures to track the estimator within the same envelope used for
the simulation tables here.
