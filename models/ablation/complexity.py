# -*- coding: utf-8 -*-
"""
FEATHer Param Complexity Ablation Model
=======================================

Variants:
- 'half': d_state // 2 (reduced parameters)
- 'full': d_state (baseline)
- 'double': d_state * 2 (increased parameters)

Usage:
    model = FEATHer_Complexity(
        seq_len=96,
        pred_len=96,
        d_model=7,
        d_state=10,
        ...
        period=12,
        num_bands=4,
        complexity='full',  # 'half', 'full', 'double'
    )
"""
...
# =============================================================================
# FEATHer Complexity Ablation
# =============================================================================

class FEATHer_Complexity(nn.Module):
    """
    FEATHer with configurable model complexity
    ...
    def __init__(
        ...
        assert complexity in ['half', 'full', 'double'], \
        ...
        # Adjust d_state based on complexity
        if complexity == 'half':
            self.d_state = max(1, d_state // 2)
        ...
        # Shared DTK (with adjusted d_state)
    ...
    def get_variant_name(self):
        return self.VARIANT_NAMES.get(self.complexity, f"Custom_{self.complexity}")
...
def get_all_variants():
    """Return all Complexity variants"""
    return ['half', 'full', 'double']


def get_variant_name(complexity):
    """Return variant name for given complexity"""
    return FEATHer_Complexity.VARIANT_NAMES.get(complexity, f"Custom_{complexity}")
